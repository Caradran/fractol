...
t_mand	ft_ship(t_cmpl z, t_mand c)
{
	double tmp;
...
void	ft_ship_mult(t_env *env)
{
	...
	while (i < THREADS)
	{
		env_i[i].part = i;
		env_i[i].env = env;
		if (pthread_create(&thread[i], NULL, ft_ship_th, &env_i[i]))
			return ;
		++i;
	}
	i = 0;
...
void	*ft_julia_ship_th(void *arg)
{
	t_env_th	*tmp;
	int			start;
	int			end;
...
void	ft_julia_ship_mult(t_env *env)
{
	...
	while (i < THREADS)
	{
		env_i[i].part = i;
		env_i[i].env = env;
		pthread_create(&thread[i], NULL, ft_julia_ship_th, &env_i[i]);
		++i;
	}
	i = 0;
	while (i < THREADS)
